...
def seriefourier(s):
    # Assicuriamoci di avere un vettore riga, ovvero un segnale mono
    if s.ndim > 1:
        s = s[:, 0]

    y = np.fft.fft(s) / len(s);

    # Ricostruiamo i coefficienti a, b dalla serie di Fourier complessa
    a = np.array(y[0] * 2, ndmin=1)

    l = int(np.floor((len(s)+1) / 2));
    
    a = np.concatenate((a, 2 * np.real(y[1 : l])));
    b = -2 * np.imag(y[1:l]);

    if len(y) % 2 == 0:
        a = np.concatenate((a , np.array(np.real(y[l+1]), ndmin=1)));
...
def suonodaserie(a, b):
    N = len(a) + len(b);

    if N % 2 == 0:
        y = np.concatenate(( [a[0] / 2], (a[1:-1] - 1j * b) / 2 , [a[-1]] ));
        y = np.concatenate(( y, np.conj(y[-1:0:-1]) ));
    else:
        y = np.concatenate(( [a[0]], a[1:] - 1j * b )) / 2;
        y = np.concatenate(( y, np.conj(y[:0:-1]) ));

    s = np.real(np.fft.ifft(y * N));
    
    return s

conv = np.convolve

mat = loadmat('filtri.mat')
